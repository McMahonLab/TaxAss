# RRR 1/17/18
# To satisfy reviewer #2, and also I think Pat Schloss bioRxiv review (and because they had a good point):
...
make.stacked.bar.for.manuscript <- function(){ # laxy calls from global env
  ...
  line.loc <- cumsum(results.v4[ ,7])
  line.loc <- line.loc - (.5 * results.v4[ ,7])
  label.loc <- seq(from = 30, to = 315, along.with = line.loc)
  
  max.val <- sum(results.v4[ ,1])
  
  par(mar = c(2,4,3,16))
  bar.loc <- barplot(results.v4[ ,-(1:4)], col = col.vector, axisnames = F, axes = F)
  
  mtext(text = colnames(results.v4)[-(1:4)], side = 1, line = .5, at = bar.loc - c(.4,0,-.1), cex = label.cex)
  
  axis(side = 2, at = c(0, max.val), labels = F)
  tic.loc <- axis(side = 2, labels = F)
  label.scooch <- c(tic.loc + c(4, rep.int(x = 0, times = length(tic.loc) - 1)) , max.val + 10) # 0 up, 285 up
  ...
  text(x = rep.int(x = 4.2, times = nrow(results.v4)), y = label.loc, labels = row.names(results.v4), xpd = T, adj = 0, cex = excel.cex, col = col.vector)
  for (n in 1:nrow(results.v4)){
...
make.example.table <- function(){ # all lazy calls to global env
  ...
  for (b in 1:length(word.heights)){
    rect(xleft = box.lefts, xright = box.middles, ybottom = box.bottoms[b], ytop = box.tops[b], xpd = T)
  }
  # right boxes
  for (b in 1:length(word.heights)){
    rect(xleft = box.middles, xright = box.rights, ybottom = box.bottoms[b], ytop = box.tops[b], xpd = T)
  }
  
  plot(x = c(0,10), y = word.heights[c(1,length(word.heights))])
