...
file.path.blast.table <- "~/Desktop/TaxonomyTrainingSets/BLASTing/take18 copy/otus.custom.blast.table.modified" # blast table w/ calculations generated by step 4
file.path.taxa.table <- "~/Desktop/TaxonomyTrainingSets/BLASTing/take18/data/otus.98.80.70.taxonomy" # final taxonomy table generated by step 15
file.path.FW_only.taxa.table <- "~/Desktop/TaxonomyTrainingSets/BLASTing/take18 copy/otus.custom.80.taxonomy" # taxonomy table of classifications using custom database only generated in step 15.5.b          
file.path.seqid.reads <- "~/Desktop/TaxonomyTrainingSets/BLASTing/take18 copy/total.reads.per.seqID.csv" # table generated by step 14
...
histogram.overlay <- function(BlastData, WorkflowData, NumBreaks, MinX = "min", PlotTitle){
  ...
  max.y <- max(hist.info.b$counts, hist.info.w$counts) + 1000 # was being stupid and cutting it off 
...
par(mar = c(10,5,4,2))
barplot(fwc.into.phyla.perc.otus, las = 2, ylab = "percent all OTUs", main = "OTUs that would be forced\nwithout the BLAST conversion", xlab = "")
title(xlab = "The Phylum they would be forced into", line = 8.5)
barplot(fwc.into.phyla.perc.reads, las = 2, ylab = "percent all reads", main = "Reads that would be forced\nwithout the BLAST conversion", xlab = "", beside = FALSE)
title(xlab = "The Phylum they would be forced into", line = 8.5)
...
cyano.forced <- find.forced.taxonomies(BlastData = cyano.blast, Cutoff = 0, FWTax = fw.taxa)
cyano.phyla <- count.up.tax.names(ForcedTax = cyano.forced, TaxLevel = 3)
barplot(cyano.phyla, beside = FALSE)
